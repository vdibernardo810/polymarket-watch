...
WALLET_NAME = "terrordogchampion"
WALLET_ADDRESS = "0x376a5fd4c4941280fbf511489558e9b7de31d270"
...
def fetch_activity_window(address, window_start_ts, max_pages=100):
    """
    Fetch BUY trade activity within the lookback window using /v2/activity,
    which is cursor-paginated with no depth limit (unlike v1's offset
    pagination, which hard-fails around ~5500 records - that's what caused
    the 90-day run to silently only cover ~27 days).

    v2 uses different field names than v1: usdc_size (not usdcSize),
    token_id (not asset), condition_id (not conditionId), outcome_index
    (not outcomeIndex) - confirmed against real API output, not assumed.
    """
    all_items = []
    cursor = None
    page_size = 500
    ...
    for _ in range(max_pages):
        url = (
            DATA_API_V2 + "/activity?user=" + address
            + "&limit=" + str(page_size)
            + "&type=TRADE"
        )
        if cursor:
...
def build_resolution_lookup(closed_positions, redeemable_positions):
    ...
    for pos in closed_positions:
        info = {"kind": "closed", "percent_pnl": pos.get("percent_realized_pnl", 0)}
        token_id = pos.get("token_id", "")
        slug = pos.get("slug", "")
        title = pos.get("title", "")
        outcome = pos.get("outcome", "")
...
def main():
    ...
    for item in trades:
        ...
        day_totals[day_key] = day_totals.get(day_key, 0) + float(item.get("usdc_size", 0) or 0)
    ...
    for item in trades:
        ...
        usdc = float(item.get("usdc_size", 0) or 0)
    ...
    # tracks each distinct token_id's outcome once, regardless of how many
    # times it was traded - this answers "how many genuinely different
    # calls did they get right" separately from the money simulation above,
    # which correctly re-sizes and re-counts every trade since that's how
    # the balance actually moves.
    unique_market_result = {}
    ...
    for item in qualifying:
        ...
        if match["kind"] == "redeemable":
            ...
            debug_info = "percent_realized_pnl=" + str(match["percent_pnl"])
        ...
        if token_id and token_id not in unique_market_result:
            if return_multiple > 1:
                unique_market_result[token_id] = "win"
            elif return_multiple < 1:
                unique_market_result[token_id] = "loss"
            else:
                unique_market_result[token_id] = "push"
    ...
    print("Trades simulated: " + str(simulated_count) + "  (Wins: " + str(wins) + "  Losses: " + str(losses) + "  Pushes: " + str(pushes) + ")")
    ...
    unique_wins = sum(1 for v in unique_market_result.values() if v == "win")
    unique_losses = sum(1 for v in unique_market_result.values() if v == "loss")
    unique_pushes = sum(1 for v in unique_market_result.values() if v == "push")
    unique_decided = unique_wins + unique_losses
    unique_win_rate = (unique_wins / unique_decided * 100) if unique_decided > 0 else 0.0

    print("Unique markets bet on (each counted once, regardless of how many times it was traded):")
    print("  " + str(len(unique_market_result)) + " unique markets  -  Wins: " + str(unique_wins) + "  Losses: " + str(unique_losses) + "  Pushes: " + str(unique_pushes))
    print("  Unique-market win rate: " + str(round(unique_win_rate, 1)) + "%")
    print("")
