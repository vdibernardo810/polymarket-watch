...
def build_resolution_lookup(closed_positions, redeemable_positions):
    """Key by (title, outcome) -> resolution info."""
    lookup = {}
    for pos in redeemable_positions:
        key = (pos.get("title", ""), pos.get("outcome", ""))
        lookup[key] = {"kind": "redeemable", "current_price": pos.get("current_price", 0)}
    for pos in closed_positions:
        key = (pos.get("title", ""), pos.get("outcome", ""))
        # don't overwrite a redeemable match with a closed one if both exist
        if key not in lookup:
            lookup[key] = {"kind": "closed", "percent_pnl": pos.get("percent_pnl", 0)}
    return lookup
...
def main():
    ...
    resolution_lookup = build_resolution_lookup(closed, redeemable)
    print("  " + str(len(resolution_lookup)) + " resolved (market, outcome) pairs available for matching")
    print("")
    ...
    for item in qualifying:
        ...
        match = resolution_lookup.get((title, outcome))
        ...
        print(date_str + "  " + result + "  " + pct_str + "% sizing  bet $" + format(bet_size, ",.2f") + "  pnl $" + format(pnl, ",.2f") + "  -> balance $" + format(balance, ",.2f") + "  [" + title[:50] + "]")
