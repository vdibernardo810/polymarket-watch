...
ACCOUNTS = {
    ...
    "terrordogchampion": "0x376a5fd4c4941280fbf511489558e9b7de31d270",
}

# Accounts in this set get flagged/prioritized in the notification
PRIORITY_ACCOUNTS = {"ferrarichampions2026"}

# Optional per-account emoji shown in the notification title, so you can
# tell accounts apart at a glance without reading the name.
ACCOUNT_EMOJIS = {
    "ferrarichampions2026": "\u2b50",  # star
    "terrordogchampion": "\u26a1",  # lightning bolt
}

# Skip notifying about bets smaller than this percentage of the account's
# total volume for the day - filters out noise from small trades.
MIN_PCT_OF_DAY = 5.0
...
def format_bet_message(item: dict, pct_of_day: float, day_total: float) -> str:
    ...
    pct_rounded = round(pct_of_day, 1)

    line1 = side + " " + outcome + " on: " + title
    line2 = "$" + format(usdc, ",.2f") + " (" + format(size, ",.1f") + " shares @ " + str(price_cents) + "c)"
    line3 = "= " + str(pct_rounded) + "% of their $" + format(day_total, ",.2f") + " total today"
...
def main():
    ...
    for name, address in ACCOUNTS.items():
        ...
        is_first_run_for_account = address not in state
        last_seen = state.get(address, {}).get("last_ts", 0)

        if is_first_run_for_account:
            # Don't flood notifications with an account's entire trade
            # history on the very first run - just record a baseline and
            # start alerting on whatever comes in after this.
            print(f"  first run for {name}: recording baseline, no notifications sent for history")
            new_trades = []
        else:
        ...
        for item in reversed(new_trades):
            ...
            if pct_of_day < MIN_PCT_OF_DAY:
                ...
                print("  skip (below " + str(MIN_PCT_OF_DAY) + "% threshold): " + pct_str + "% - " + item.get("title", ""))
                continue

            is_priority = name in PRIORITY_ACCOUNTS
            emoji = ACCOUNT_EMOJIS.get(name, "")
            emoji_prefix = (emoji + " ") if emoji else ""
            title = emoji_prefix + name + " placed a bet"
